...
export class TablesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.service.getAllFinancias().subscribe((data) => {
      ...
      const formatDados = this.formatIsPago(data)
      this.dataSource.data = formatDados;
  ...
  private formatIsPago(isPago: Financias[]) {
    return isPago.map(item => ({
      ...item,
      isPago: item.isPago ? "Sim" : "Não"
    }))
  }
